...
EOD;
	...
	{
		if(!isset($htmlOptions['submit']) && !isset($htmlOptions['confirm']) && !isset($htmlOptions['ajax']) && !isset($htmlOptions['on'.$event]))
			return;
		...
		if($live)
			$cs->registerScript('Yii.CHtml.#' . $id, "jQuery('body').on('$event','#$id',function(event){{$handler}});");
		else
			$cs->registerScript('Yii.CHtml.#' . $id, "jQuery('#$id').on('$event', function(event){{$handler}});");
